...
// #define MCP2515_SELECT()   GPIO_setLow(obj->gpioHandle, handle->gpio_CS)
//
// #define MCP2515_UNSELECT() GPIO_setHigh(obj->gpioHandle, handle->gpio_CS)
//
...
/*********************************************************************************************************
** Function name:           MCP2515_write_id_N
** Descriptions:            write can id
** without RAM
** input parameters:        NONE
** Output parameters:       NONE
** Returned value:          NONE
*********************************************************************************************************/
void MCP2515_write_id_N(MCP2515_Handle handle,
                        const uint8_t mcp_addr,
                        const uint8_t ext,
                        const uint32_t id) {
    if (ext == 1) {
        MCP2515_setRegister(handle, mcp_addr, (uint8_t)(id >> 21));                                                  // TXBnSIDH: старшие 8 бит (28-21)
        MCP2515_setRegister(handle, mcp_addr + 1, (uint8_t)(((id >> 16) & 0x03) | ((id >> 13) & 0xE0) | (1 << 3)));  // TXBnSIDL: младшие 3 бита стандартного ID + EFF
        MCP2515_setRegister(handle, mcp_addr + 2, (uint8_t)(id >> 8));                                               // TXBnEID8: старшие 8 бит расширенного ID (20-13)
        MCP2515_setRegister(handle, mcp_addr + 3, (uint8_t)(id));                                                    // TXBnEID0: младшие 8 бит расширенного ID (12-0)

    } else {
        MCP2515_setRegister(handle, mcp_addr, (id >> 3) & 0xFF);      // SIDH
        MCP2515_setRegister(handle, mcp_addr + 1, (id << 5) & 0xE0);  // SIDL
    }
}
...
void MCP2515_initCANBuffers(MCP2515_Handle handle) {
    ...
    MCP2515_write_id(handle, MCP_RXF1SIDH, std, ulFilt); /* AND standard                 */
    MCP2515_write_id(handle, MCP_RXF2SIDH, ext, ulFilt); /* RXB1: extended               */
    MCP2515_write_id(handle, MCP_RXF3SIDH, std, ulFilt); /* AND standard                 */
    MCP2515_write_id(handle, MCP_RXF4SIDH, ext, ulFilt);
    MCP2515_write_id(handle, MCP_RXF5SIDH, std, ulFilt);
...
void MCP2515_read_id(MCP2515_Handle handle,
    ...
    *id = (tbufdata[MCP_SIDH] << 3) + (tbufdata[MCP_SIDL] >> 5);
    ...
    if ((tbufdata[MCP_SIDL] & MCP_TXB_EXIDE_M) == MCP_TXB_EXIDE_M) {
        ...
        *id <<= 16;
        *id = *id + ((uint32_t)tbufdata[MCP_EID8] << 8) + tbufdata[MCP_EID0];
        *ext = 1;
    }
}
...
void MCP2515_write_canMsg(MCP2515_Handle handle, const uint8_t buffer_sidh_addr) {
    ...
    // MCP2515_write_id_N(handle, mcp_addr, m_nExtFlg, m_nID);
}
...
uint8_t MCP2515_prepare(MCP2515_Handle handle, const uint8_t canSpeed) /* MCP2515init                  */
{
    ...
    if (res == MCP2515_OK) {
        ...
        /* interrupt mode               */
        MCP2515_setRegister(handle, MCP_CANINTE, MCP_RX0IE | MCP_RX1IE);
        // Включить прерывания на приём данных в RX0 и RX1, а также успешную передачу TX0, TX1, TX2
        // MCP2515_setRegister(handle, MCP_CANINTE, MCP_RX0IE | MCP_RX1IE | MCP_TX0IE | MCP_TX1IE | MCP_TX2IE);
...
uint8_t MCP2515_sendMsg(MCP2515_Handle handle) {
    ...
    do {
        uiTimeOut++;
        res1 = MCP2515_readRegister(handle, txbuf_n);  // read send buff ctrl reg
        res1 = res1 & 0x08;
...
/*********************************************************************************************************
** Function name:           sendMsgN
** Descriptions:            send message to N buffer
** input parameters:        NONE
** Output parameters:       NONE
** Returned value:          if success
*********************************************************************************************************/
uint8_t MCP2515_sendMsgN(MCP2515_Handle handle, uint8_t num) {
    uint8_t address;
    switch (num) {
        case 0:
            address = MCP_TXB0SIDH;
            break;
        case 1:
            address = MCP_TXB1SIDH;
            break;
        case 2:
            address = MCP_TXB2SIDH;
            break;
        default:
            return CAN_FAILTX;
    }
    MCP2515_write_canMsg(handle, address);
    MCP2515_start_transmit(handle, address);
    return CAN_OK;
}
...
/*********************************************************************************************************
** Function name:           sendMsgBufN
** Descriptions:            send buf to N buffer
** input parameters:        id  :   can id
                            ext :   if ext
                            len :   buf length
                            *buf:   data buf
...
** Returned value:          if success
*********************************************************************************************************/
uint8_t MCP2515_sendMsgBufN(MCP2515_Handle handle, uint32_t id, uint8_t ext, uint8_t len, uint8_t *buf, uint8_t num) {
    MCP2515_setMsg(id, ext, len, buf);
    return MCP2515_sendMsgN(handle, num);
}
...
uint8_t MCP2515_readMsg(MCP2515_Handle handle) {
    ...
    if (stat & MCP_STAT_RX0IF) { /* Msg in Buffer 0              */
        ...
        MCP2515_modifyRegister(handle, MCP_CANINTF, MCP_RX0IF, 0x00);
        res = CAN_OK;
    } else if (stat & MCP_STAT_RX1IF) { /* Msg in Buffer 1              */
        MCP2515_read_canMsg(handle, MCP_RXBUF_1);
        MCP2515_modifyRegister(handle, MCP_CANINTF, MCP_RX1IF, 0x00);
        res = CAN_OK;
    } else {
        res = CAN_NOMSG;
    }
...
/*********************************************************************************************************
** Function name:           readMsg
** Descriptions:            read message from buffer N
** input parameters:        NONE
** Output parameters:       NONE
** Returned value:          if success
*********************************************************************************************************/
uint8_t MCP2515_readMsgN(MCP2515_Handle handle, uint8_t stat) {
    uint8_t res;
...
/*********************************************************************************************************
** Function name:           readMsgBuf
** Descriptions:            read message from buf N
** input parameters:        len:    data length
** Output parameters:       buf:    data buf
** Returned value:          if success
*********************************************************************************************************/
uint8_t MCP2515_readMsgBufN(MCP2515_Handle handle, uint8_t *len, uint8_t buf[], uint8_t num) {
    int res = MCP2515_readMsgN(handle, num);
    if (res != CAN_OK) {
        *len = 0;
        return res;
    }
...
// #ifdef DEBUG
/*********************************************************************************************************
** Function name:           MCP2515_getStateAfterRead
** Descriptions:            returns interrupt pin state
** input parameters:        NONE
** Output parameters:       NONE
** Returned value:          0/1
*********************************************************************************************************/
// uint16_t MCP2515_getStateAfterRead(){
//	return interruptPinStateAfterReadMsg;
// }

/*********************************************************************************************************
** Function name:           MCP2515_setStateAfterRead
** Descriptions:            (re)sets the interruptPinStateAfterReadMsg variable
** input parameters:        0/1 (1 default)
** Output parameters:       NONE
** Returned value:          NONE
*********************************************************************************************************/
// void MCP2515_setStateAfterRead(uint16_t state){
//	interruptPinStateAfterReadMsg = state;
// }
// #endif
/*********************************************************************************************************
  END FILE
