...
try:
    ...
    if args.mode == "write":
        ...
        # Проверка записанных параметров
        while True:
            verification_failed = False
            
            # Находим различия
            diff_dict = {
                key: param_index[key] for key in send_param if key not in dic_param or dic_param[key] != send_param[key]
            }

            # Отправка параметров из diff_dict
            for key, value in diff_dict.items():
                if key == "MIDLE_POINT":
                    parts = send_param["MIDLE_POINT"]
                    midle_point_value = 1 if parts[0] else 0
                    esc_min_value = int(parts[1]) & 0x3FF  # 10 бит (макс. 1023)
                    esc_max_value = int(parts[2]) & 0x1FFF  # 13 бит (макс. 8191)
                    ...
                    value_to_send = param_type(value[0]) if isinstance(value, list) else param_type(value)
                    send_value(param_index[key][0], value_to_send)

                node.spin(timeout=1.5)

                # Проверка записанных параметров
                expected_value = [str(param_type(value[0]))]  # Приводим к строковому представлению
                if dic_param.get(key) != expected_value:
                    print(f"Warning: Verification failed for {key}. Expected {expected_value}, got {dic_param.get(key)}")
                    verification_failed = True

            if not verification_failed:
                print("All parameters verified successfully.")
                break  # Выход из цикла, если все параметры записаны правильно
